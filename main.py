...
from airthings import AirthingsConnector
...
def get_config():
    ...
    try:
        with open(Path(__file__).with_name(CONFIG_FILE)) as config_file:
            ...
            for name in {"influx", "airthings", "main"}:
...
try:
    ...
    airthingsConnector = AirthingsConnector(config["airthings"])

    while True:
        try:
            records = airthingsConnector.fetch_data()
